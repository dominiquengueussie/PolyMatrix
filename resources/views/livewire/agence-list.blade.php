                                    ...
                                    <tbody class="text-center">
                                        @foreach ($agences as $agence)
                                            <tr>
                                                ...
                                                <td>{{ $agence->nom }}</td>
                                                <td>
                                                    <form action="{{ route('agence.destroy', $agence->id) }}"
                                                        method="POST">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header bg-success">
                                        <h5 class="modal-title" id="exampleModalLabel">Créer une agence</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form class="row g-3" action="{{ route('agence.store') }}" method="post">
                                            @csrf
                                            <div class="col-auto">
                                                <label for="nom" class="visually-hidden">Agence</label>
                                                <input type="text"
                                                    class="form-control @error('nom') is-invalid @enderror"
                                                    size="50" value="{{ old('nom') }}" name="nom"
                                                    id="nom" placeholder="agence">
                                                @if ($errors->has('nom'))
