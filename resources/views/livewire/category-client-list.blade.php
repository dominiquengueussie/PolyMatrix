                                    ...
                                    <tbody class="text-center">
                                        @foreach ($categorys as $category)
                                            <tr>
                                                ...
                                                <td>{{ $category->nom }}</td>
                                                <td>
                                                    <form action="{{ route('category_client.destroy', $category->id) }}"
                                                        method="POST">
                                                        <a title="Modifier agence" class=""
                                                            href="{{ route('category_client.edit', $category->id) }}"><i
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header bg-success">
                                        <h5 class="modal-title" id="exampleModalLabel">Créer une catégorie de client</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form class="row g-3" action="{{ route('category_client.store') }}" method="post">
                                            @csrf
                                            <div class="col-auto">
                                                <label for="nom" class="visually-hidden">Catégorie de client</label>
                                                <input type="text"
