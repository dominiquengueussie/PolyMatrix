                                        ...
                                        <tbody class="text-center">
                                            @foreach ($zones as $zone)
                                                <tr>
                                                    ...
                                                    <td>{{ $zone->nom }}</td>
                                                    <td>
                                                        <form action="{{ route('zone_commerciale.destroy', $zone->id) }}"
                                                            method="POST">
                                                            <a title="Modifier agence" class=""
                                                                href="{{ route('zone_commerciale.edit', $zone->id) }}"><i
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header bg-success">
                                            <h5 class="modal-title" id="exampleModalLabel">Créer une zone commerciale</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <form class="row g-3" action="{{ route('zone_commerciale.store') }}" method="post">
                                                @csrf
                                                <div class="col-auto">
                                                    <label for="nom" class="visually-hidden">Zone commerciale</label>
                                                    <input type="text"
                                                        class="form-control @error('nom') is-invalid @enderror"
                                                        size="50" value="{{ old('nom') }}" name="nom"
                                                        id="nom" placeholder="Exemple: Douala, Hôpital général">
                                                    @if ($errors->has('nom'))
